...
pub fn init_debug_commands(cmds: &mut CommandCenter<App>) {
    cmds.add_command("f", "freeze the app", |app, _| app.freeze = true);
    cmds.add_command("uf", "unfreeze the app", |app, _| app.freeze = false);
    cmds.add_command("hw", "hide the world rendering", |app, _| {
        app.draw_world = false
    });
    cmds.add_command("sw", "show the world rendering", |app, _| {
        app.draw_world = true
    });
    cmds.add_command("reset", "reset app back to the start state", |app, _| {
    ...
    cmds.add_command("ddl", "list all debug draws", |app, _| {
        for key in app.debug_draws.keys() {
            info!("{}", key);
        }
    });
}
