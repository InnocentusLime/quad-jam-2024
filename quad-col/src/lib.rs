mod shape_collisions;

use glam::{Affine2, Vec2};
use shape_collisions::*;
...
impl Collider {
    pub fn collides(&self, other: &Self) -> bool {
        ...
        match (&self.shape, &other.shape) {
            (Shape::Rect { size: wh1 }, Shape::Rect { size: wh2 }) => {
                collision_rect_rect(self.tf, *wh1, other.tf, *wh2)
            }
            (Shape::Rect { size }, Shape::Circle { radius }) => {
                collision_circle_rect(other.tf, *radius, self.tf, *size)
            }
            (Shape::Circle { radius }, Shape::Rect { size }) => {
                collision_circle_rect(self.tf, *radius, other.tf, *size)
            }
            (Shape::Circle { radius: r1 }, Shape::Circle { radius: r2 }) => {
                collision_circle_circle(self.tf, *r1, other.tf, *r2)
            }
        }
    }
}
