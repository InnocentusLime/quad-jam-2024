...
impl Shape {
    ...
    pub fn separating_axes(
        ...
        tf1: Affine2,
        tf2: Affine2,
        out: &mut [Vec2; MAX_AXIS_NORMALS],
    ) -> usize {
        let n = match (*self, *other) {
            (Shape::Circle { .. }, Shape::Circle { .. }) => {
                let center1 = tf1.transform_point2(Vec2::ZERO);
                let center2 = tf2.transform_point2(Vec2::ZERO);
                out[0] = (center1 - center2).normalize_or_zero();
                1
            }
            (Shape::Rect { .. }, Shape::Rect { .. }) => {
                out[0..4].copy_from_slice(&rect_normals(tf1));
                out[4..8].copy_from_slice(&rect_normals(tf2));
                8
            }
            (Shape::Rect { width, height }, Shape::Circle { .. }) => {
                out[0..4].copy_from_slice(&rect_normals(tf1));
                out[4] = rect_circle_extra_separating_axis(vec2(width, height), tf1, tf2);
                5
            }
            (Shape::Circle { .. }, Shape::Rect { width, height }) => {
                out[0..4].copy_from_slice(&rect_normals(tf2));
                out[4] = rect_circle_extra_separating_axis(vec2(width, height), tf2, tf1);
                5
            }
        };
        debug_assert!(n <= MAX_AXIS_NORMALS, "too many normals");
...
/// When applying the separating axis theorem to a circle
/// and a rectangle, there's an edge case when the rectangle is rotated
/// with its vertex being the closest point to the circle.
///
/// For this case, the best separating axis candidate would
/// be an axis perpendicular to the line connecting the circle
/// center with that closest vertex.
///
/// * `rectangle_size` -- the rectangle sides (width, height)
/// * `rectangle_tf` -- rectangle transform
/// * `circle_tf` -- circle transform
pub fn rect_circle_extra_separating_axis(
    rectangle_size: Vec2,
    rectangle_tf: Affine2,
    circle_tf: Affine2,
) -> Vec2 {
    let circle_center = circle_tf.transform_point2(Vec2::ZERO);
    let rect_points = rect_points(rectangle_size, rectangle_tf);
    let distances = vec4(
        rect_points[0].distance(circle_center),
        rect_points[1].distance(circle_center),
        rect_points[2].distance(circle_center),
        rect_points[3].distance(circle_center),
    );
    let closest_point = rect_points[distances.min_position()];
